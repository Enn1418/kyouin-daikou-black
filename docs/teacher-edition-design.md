...
| `board` | JSON（columns / items） | 板書計画。3600×1200mm。別項 |

### 画像として取り出す

印刷用 HTML の各図の下に「画像として保存」が付く。押すと PNG がダウンロードされる。
掲示・投影・ロイロノート・Canva に貼るときはこれを使う。

変換は**担任のブラウザ自身が行う**。SVG から PNG への変換には普通は変換ソフトが要るが、
それを導入させると「動かすまで」が長くなる。Chrome の canvas に描いて `toBlob` で出せば、
追加の導入なしで済む。ブリッジ側に画像処理の依存を持ち込まずに済む利点もある。

倍率は4倍。板書（900×300 で描いている）なら **3600×1200px**、
つまり実物の 3600×1200mm と 1px = 1mm で対応する。投影にも A3 印刷にも足りる。
背景は白で塗ってから描く（透明のままだと貼り先によっては黒く出る）。
ボタンは `@media print` で消してあるので、紙には写らない。
...
最初の版は「区画に見出しを置いた帯」で、板書計画として使いものにならなかった。
調べ直して分かったこと（出典は下記）:

- **1時間の流れが1枚に残る**。途中で消さない。単元名・学習問題・めあて・児童の考え・まとめ・振り返り
- **めあては青枠、まとめは赤枠**。強調は黄。文字は白70%・黄30%。**赤い文字は黒板で見えにくい**ので使わない
- **文字の大きさは学年で決まる** — 低学年 12cm角 / 中学年 10cm / 高学年 8cm
- 特別支援では**カード化・構造化**。刺激を減らす（区画を増やしすぎない）

最後の2つが、コードが引き受けるところ。12cm角の文字なら 1200mm の黒板に縦10行しか入らない。
**入りきるかは数えれば分かる**ので、書ききれない板書計画を黙って出さない
（溢れた区画を赤枠で囲み、何mm分あふれたかを書く）。

指示は入れ子になるので、`図` ブロックの中身を JSON で書く。

    ```図
    {"type":"board","grade":"低学年","header":"8/25 かさ",
     "columns":[{"width":2,"items":[
       {"kind":"label","text":"めあて"},
       {"kind":"box","frame":"blue","text":"どちらが おおいか くらべよう"}]}]}
    ```

`kind` は label（区画の見出し）/ box（枠つき。frame は blue・red・yellow）/ card（薄い枠）/
bubble（児童の考えカード）/ text / **figure（区画の中に上記の図を入れる）**。

出典（2026-08-22 に確認）:
[構造的な板書の5つのポイント](https://edulo.jp/bansyo-pointo/) /
[香川県教育委員会・板書の基本](https://www.pref.kagawa.lg.jp/documents/14668/12-13bansyo.pdf) /
[図解によるビジュアル板書の作り方](https://www.meijitosho.co.jp/eduzine/opinion/?id=20130390)

### 作る手順を3つに割った

担任の提案で、板書を1人に任せるのをやめた（`sn-board`「特支・板書室」）。

| 役 | 担当 |
|---|---|
| 板書デザイナー | 実態と参加児童を確認／**2段階に分けて発注**／最後に検査 |
| 板書リサーチ | 当該単元の板書実践を調べ、構成の型を2〜3案。**出典URL必須**。web 検索あり |
| 配置設計 | 区画の並び・幅の比・各区画の文字。**文字数を数えて削る** |
| 図解づくり | 割り付けを board の JSON にする。「入りません」が出たら削って作り直す |

リサーチが終わる前に配置と図解を動かすと、根拠のない配置になる。
だからリードは**第1段でリサーチだけ**を出し、返ってきてから残り2人に流す。

## 手持ちの資料を読ませる（参照フォルダ）

担任はすでに資料を持っている。Obsidian の保管庫、NotebookLM に入れた資料、
Google ドライブのフォルダ。これを使えないと、毎回ゼロから作ることになる。

### Obsidian — そのまま繋がる

保管庫は**ただの Markdown フォルダ**なので、読めるようにするだけでよい。

```
npm run bridge -- --root "…\kyouin" --ref "ノート=…\Obsidian\授業"
```

- `--ref` は何個でも並べられる。名前はエージェントが指定に使う
- **参照フォルダは読み取り専用**。書き込みは断る。担任が長年ためたノートを、
  こちらの都合で書き換える余地を残さない
- `.obsidian` などの隠しフォルダは見ない

### NotebookLM — 一般向けの API は無い（2026-08 時点）

調べた結果、一般向けの版に公開 API は無く、あるのは企業向けの
Gemini Notebook Enterprise（Google Cloud プロジェクトと有償ライセンスが要る）。
非公開のエンドポイントを叩く非公式ライブラリはあるが、
**担任が毎日使う道具の土台にはしない**（予告なく壊れるし、規約上の位置づけも曖昧）。

代わりの道:

1. NotebookLM の中身は**元資料が本体**。その元資料を参照フォルダに置く
2. NotebookLM で作った要約・ノートは、Google ドキュメントに書き出す →
   **Google ドライブ デスクトップ**でPCに同期 → そのフォルダを `--ref` で足す

出典: [Gemini Notebook API の現状](https://web-clipper-for-notebooklm.com/blog/notebooklm-api) /
[Gemini Notebook Enterprise の API 文書](https://docs.cloud.google.com/gemini/enterprise/notebooklm-enterprise/docs/api-notebooks)

### 探す（`search_files`）

保管庫は数千ファイルになる。一覧を渡して読ませるのは成り立たないので、
**語で探して、当たった箇所の前後だけ返す**。どれを読むかはエージェントが選ぶ。

- 語を空白区切りで複数書くと「すべて含む」で絞られる
- 隠しフォルダは見ない。1ファイル 512KB まで、4000ファイルまで走査
- 参照フォルダがあるときだけツールに載る

**読めるのは `.md .txt .csv .json .html` だけ。** PDF・Word・PowerPoint は読めない
（ブリッジに依存を入れない方針のため。必要になったら、そのとき考える）。
