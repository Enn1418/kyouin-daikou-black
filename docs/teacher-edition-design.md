...
| 12 | **ドリル生成 API**（型を渡すと数値違いを決定的に量産。答えは検算つき） | ブリッジ `POST /generate/drill` | M |
| 13 | **印刷テンプレート**（マス目・なぞり書き・分かち書き・1課題1ページ・UDフォント） | `03_印刷テンプレート/*.css`＋`/export` | M |

**着手順**: 1 → 2 →（ここで 3 段階生成が画面上で成立）→ 3 → 4 → 5 →（ここで"作業場"になる）→ 13 → 6 → 12 → 7・8 → 9。
