...
Obsidian の保管庫や、Google ドライブの同期フォルダを **読み取り専用で** 足せます。

```powershell
npm run bridge -- --root "$env:USERPROFILE\kyouin" --ref "ノート=C:\Users\まどか\Obsidian\授業"
```

`--ref "名前=パス"` を並べれば何個でも足せます。名前は自由で、エージェントはこの名前で指定します。
**参照フォルダには絶対に書き込みません**（試みると断ります）。長年のノートを壊さないためです。

読めるのは `.md .txt .csv .json .html` だけです。PDF や Word は読めません。
...
**ZIP は常に「その時点の全部」です。**途中の版を順番に入れる必要はありません。
いつ入れ替えても、一度で最新になります。

`kyouin-app2\scripts\` の中にある **更新.bat** と **起動.bat** を、
**1つ上のフォルダ**（`まなびノート`）にコピーしておいてください。以後はこうなります。

| したいこと | すること |
|---|---|
| 最新にする | **更新.bat** をダブルクリック（窓は先に閉じる） |
| 使い始める | **起動.bat** をダブルクリック |

更新しても、教材フォルダ・APIキー・ブリッジのトークンはそのままです。
入れ直す必要はありません。

手持ちの資料（Obsidian など）を読ませたいときは、**起動.bat** をメモ帳で開き、
`set "REFPATH="` の下の行の `rem ` を外してパスを書き換えてください。
