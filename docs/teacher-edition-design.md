...
ここを先に決めておかないと、エージェントは毎回ゼロから作文します。フォルダ構造そのものが、実質的にエージェントへの指示になります。

```
D:\kyouin\
  00_共通\
    学校要覧.md          … 学校の規模・行事・時程（毎回参照される前提知識）
    年間指導計画.md
    書式\               … おたより・行事要項の雛形（文体の見本になる）
  01_授業\<教科>\<単元>\
    単元計画.md
    本時案.md
    ワークシート.md
  ...
  04_校務\
  99_記憶\
    memory.md            … 「この学校ではこう」「この文体で」の蓄積
```

**`00_共通` と `99_記憶` が効きます。** 毎回同じ説明を書かなくて済むようになる ＝ R1（時間）への直接的な回答です。

---

## 6. エージェント編成とモデルの割り当て

既存の型（リード 1 ＋ サブ最大 3、`MAX_AGENTS = 5`）にそのまま収まります。`src/data/agents.ts` の `AGENTIC_SETS` に追加する形です。

| # | チーム | リード | サブエージェント | 主な成果物 |
|---|---|---|---|---|
| 1 | 授業準備室 | 単元デザイナー | 教材作成／発問設計／評価規準 | 単元計画・本時案・ワークシート・ルーブリック |
| 2 | 評価サポート室 | 評価主任 | 問題作成／解答解説／観点別整理 | 小テスト・解説・観点別の整理案 |
| 3 | 連絡・おたより室 | 広報担当 | 文面作成／読みやすさ校正／配慮チェック | 学級通信・保護者向け文書・行事案内 |
| 4 | 校務事務室 | 事務調整 | 様式作成／集計整理／日程調整 | 行事要項・アンケート設問・部活動計画 |
| 5 | 教材リフォーム室 | 改訂担当 | 差分抽出／今年度版作成／整合チェック | 昨年度教材の今年度版（**フォルダ読み取りが前提**） |

チーム 5 はローカルブリッジがあって初めて成立するチームで、実際に一番効くのはここだと考えています。ゼロから作るより、**去年の自分の教材を今年用に直す**ほうが教員の実作業に近いためです。
...
**1 件あたりの目安**: 指導案 1 本を 4 エージェントで作る場合、履歴の再送を含めて入力 5〜7 万トークン、出力 1 万トークン前後。

- 全部 Sonnet 5 → 約 $0.35（1 ドル 150 円換算で **50 円強**）
- リード Opus 5 ＋ サブ Haiku 4.5 → 同程度か、やや安い

**月 30 件作っても 1,500〜2,500 円程度**。これは「自腹で払えるか」の判断材料になる水準です。
...
| # | やること | 対象 | 規模 |
|---|---|---|---|
| 1 | 教員向け 5 チームの定義 | `src/data/teacherAgents.ts`（新規）→ `AGENTIC_SETS` に合流 | S |
| 2 | 日本語固定・教育文脈のシステムプロンプト（学習指導要領の観点、断定的評価を書かない、出典を明示） | `src/core/agent/PromptBuilder.ts` | S |
| 3 | **ローカルブリッジ**（Node、ルート配下限定、トークン認証） | `bridge/server.mjs`（新規）＋ `package.json` にスクリプト | **M** |
| 4 | **ファイル系ツール 3 本**（`list_files` / `read_file` / `write_file`） | `src/core/agent/tools/`＋`ToolRegistry.ts` | M |
| 5 | 成果物のファイル保存（画面表示と同時に書き出し） | `completeTask.ts` / `deliverProject.ts` / `FinalOutputModal.tsx` | S |
| 6 | 長期記憶（`99_記憶/memory.md` の読み書きと、プロンプトへの注入） | `PromptBuilder.ts`＋ブリッジ `/memory` | M |
| 7 | 月次コスト上限と警告 | `pricing.ts`＋`coreStore` | S |
| 8 | プロンプトキャッシュ対応 | `src/core/llm/providers/ClaudeProvider.ts` | S |
| 9 | 印刷用 HTML / `.docx` 書き出し | ブリッジ `/export`＋`FinalOutputModal.tsx` | M |
| 10 | 個人情報の入力に対する注意喚起（検出ではなく、入力欄の但し書きと初回の確認） | `src/interface/school/*`（新規） | S |
| 11 | ブリッジ未起動時のフォールバック（ファイル機能だけ無効化） | `useChatAvailability.ts` 相当のフック | S |

**着手順**: 1 → 2 →（ここで単発利用は完成）→ 3 → 4 → 5 →（ここで"作業場"になる）→ 6 → 7・8 → 9。
...
|---|---|---|---|
| **P0** | 週末 1 回 | 実装 1・2 | ブラウザで指導案・おたよりの下書きが作れる（コピペ運用） |
| **P1** | 2〜3 週 | 実装 3・4・5・11 | 教材フォルダに直接保存。去年の教材を読ませて改訂できる |
| **P2** | ＋2 週 | 実装 6・7・8・9 | 学校の文脈を記憶し、印刷用に書き出せる。費用も見える |
